...
def Model(X, T, energy, alpha, beta, gamma, contamination, Dthr):
    ...
    for point in point_list:
        ...
        # TODO 修改处
        viscosity = preprocess_value(point.viscosity)
        # TODO 得分 原始如注释
        outlier_list.append(np.log(point.viscosity)*point.detected_time+point.density)
        # outlier_list.append(viscosity*point.detected_time*point.density)
...
# standard_scaler.fit(data_origin[:, :data_origin.shape[1] - 1])
# standard_scaler_data = standard_scaler.transform(data_origin[:, :data_origin.shape[1] - 1])
#
# # 归一化
# min_max_scaler = preprocessing.MinMaxScaler(feature_range=(0, 1))
# min_max_scaler.fit(data_origin[:, :data_origin.shape[1] - 1])
# min_max_scaler_data = min_max_scaler.transform(data_origin[:, :data_origin.shape[1] - 1])
#
# # 鲁棒化
# robust_scaler = preprocessing.RobustScaler()
# robust_scaler.fit(data_origin[:, :data_origin.shape[1] - 1])
# robust_scaler_data = robust_scaler.transform(data_origin[:, :data_origin.shape[1] - 1])


# # TODO 测试


# X, y_type = make_blobs(n_samples=300, centers=3, random_state=50)    # 生成数据集
# dataPreprocessing.scatterPlot2(X)   # 生成数据集对应散点图
# Model(X, T=50, energy=5, alpha=0.95, beta=0.8, gamma=0.8, contamination=0.3, Dthr=10)
# #

#
#     :param X: 数据集
#     :param spider_num:蜘蛛数量
#     :param T: 迭代轮数
#     :param view_fild:蜘蛛观测范围（控制起始蜘蛛网大小）
#     :param energy: 蜘蛛起始能量
#     :param alpha: 蛛网变化速率
#     :param beta: 控制蜘蛛的满足度阈值
#     :param gamma: 蜘蛛能量变化速率